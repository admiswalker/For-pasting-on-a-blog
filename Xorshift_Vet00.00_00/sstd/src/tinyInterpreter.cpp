...
std::string sstd::readAll(const char* pReadFile){
	...
	sstd::file fp;
	if(!fp.fopen(pReadFile, "rb")){ sstd::pdbg_always_stop_exit("fopen was failed.\n"); }

	size_t size;
	fp.fseek(0, SEEK_END);	//ファイルポインタをファイルの最後に移動させる。
	size = fp.ftell(); 		//ファイルサイズを取得する。// fgetpos(fp,&size);
	fp.fseek(0L, SEEK_SET);	//ファイルポインタを先頭に戻す。

	std::string str(size+1, 0);	//0で初期化	//終端コード分を余分に確保

	if(fp.fread(&str[0], sizeof(char), size)!=size){ sstd::pdbg_always_stop_exit("fread was failed.\n"); }
...
std::string sstd::readAll_withoutBOM(const char* pReadFile){
	...
	size_t size;
	fp.fseek(0, SEEK_END);			//ファイルポインタをファイルの最後に移動させる。
	size = fp.ftell() - BOM_len; 	//ファイルサイズを取得する。// fgetpos(fp,&size);
