...
private:
	...
	uint rowNum;	// 行数
	uint colNum;	// 列数
	sstd::mat<uint64> bitMat8x8;	// 8x8 の BitMartix 行列の集合
	
public:
	bmat(): rowNum(0), colNum(0) {}
	bmat(const uint& row, const uint& col): rowNum(row), colNum(col), bitMat8x8((rowNum-1)/8+1, (colNum-1)/8+1) {}
	...
	// inline sstd::mat<uint64> bMat8x8(){ return bitMat8x8; } // Do not do this to avoid object copy
	inline const sstd::mat<uint64>& bMat8x8_R() const { return bitMat8x8; }
	inline const uint rows() const { return rowNum; }
	inline const uint cols() const { return colNum; }

	// RW: read and write
	inline sstd::mat<uint64>& bMat8x8_RW(){ return bitMat8x8; }
